...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if ( (calibratingA>0 && ACC ) || (calibratingG>0) ) { // Calibration phasis
    ...
    if (f.ARMED) {
      if (analog.vbat>VBATLEVEL_WARN1) {
        LEDPIN_ON;
      } else if (analog.vbat>VBATLEVEL_WARN2) {
        if(lowBatBlinkDelta&0x80) { LEDPIN_ON; } else { LEDPIN_OFF; }
        lowBatBlinkDelta++;
      } else if (analog.vbat>VBATLEVEL_CRIT) {
        if(lowBatBlinkDelta&0x40) { LEDPIN_ON; } else { LEDPIN_OFF; }
        lowBatBlinkDelta++;
      } else {
        if(lowBatBlinkDelta&0x20) { LEDPIN_ON; } else { LEDPIN_OFF; }
        lowBatBlinkDelta++;
      }
    }
  }
  ...
  if (f.ARMED)  {
      ...
      #if defined(POT_A)
        int ku, kp;
        if(constrain(rcData[POT_G],1000,2000) < 1300) {
          ku = ((constrain(rcData[POT_A],1000,2000) -1000) << 1) >> 6 ; // * 20/640;
          kp = ku + 8; //range: 1.0-3.7
...
void loop () {
 ...
 //*********************************** 
  ...
  if ( f.HORIZON_MODE ) {
    prop = min(max(abs(rcCommand[PITCH]),abs(rcCommand[ROLL])),512); // original code
    prop = prop + (min(max(abs(att.angle[PITCH]),abs(att.angle[ROLL])),1800)>>3); //new
    prop = min(prop,512); // new, ensure limited to 512
  }
